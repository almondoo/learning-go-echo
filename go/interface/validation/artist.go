...
//- ユーザーログイン時のリクエスト
type ArtistLoginRequest struct {
	Email    string `json:"email" form:"email" validate:"required,email"`
	Password string `json:"password" form:"password" validate:"required"`
}

func ArtistLoginMessage(err error) map[string]string {
...
func ArtistRegisterMessage(err error) map[string]string {
	var errorMessages = make(map[string]string)
	errors := err.(validator.ValidationErrors)
	if len(errors) != 0 {
		for i := range errors {
...
type ArtistEditRequest struct {
	Name  string `json:"name" form:"name" validate:"required,max=30"`
	Email string `json:"email" form:"email" validate:"required,email,max=255"`
}

func ArtistEditMessage(err error) map[string]string {
	var errorMessages = make(map[string]string)
	errors := err.(validator.ValidationErrors)
	if len(errors) != 0 {
		for i := range errors {
